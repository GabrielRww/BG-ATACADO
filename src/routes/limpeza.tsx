...
import { ArrowLeft, MessageCircle, Search, Package, Sparkles, ShoppingCart } from "lucide-react";
...
function ProductCard({ p }: { p: Produto }) {
  ...
  function adicionar() {
    addItem({
      ...
      variante:
        variante && variante.volume.toLowerCase() !== "único" ? variante.volume : null,
      preco_unit: precoAtual,
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        <Link
          to="/produto/$id"
          params={{ id: p.id }}
          className="font-display tracking-tight font-bold text-[15px] leading-snug text-foreground line-clamp-2 hover:text-primary-dark transition-colors"
        >
          {nomeLimpo(p.nome)}
        </Link>
        {p.descricao && (
        ...
        {precoAtual > 0 ? (
          <div className="mt-3 flex gap-2">
            <Link
              to="/produto/$id"
              params={{ id: p.id }}
              className="inline-flex items-center justify-center rounded-xl px-3 py-2.5 text-sm font-semibold ring-1 ring-black/10 text-foreground/70 hover:border-primary-dark/40 transition-colors"
            >
              Ver
            </Link>
            <button
              type="button"
              onClick={adicionar}
              className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold text-white transition-all hover:scale-[1.02]"
